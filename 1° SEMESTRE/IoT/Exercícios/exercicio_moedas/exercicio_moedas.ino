...
void loop() {
  if ((millis() - lastTime) > timerDelay) {
    if (WiFi.status() == WL_CONNECTED) {
      ...
      // Dólar:
      String bidUSDStr = myObject["USDBRL"]["bid"];
      float dolar = bidUSDStr.toFloat();
      Serial.print("Dólar: ");
      Serial.print("$ ");
      Serial.println(dolar, 2);

      // Euro:
      String bidEURStr = myObject["EURBRL"]["bid"];
      float euro = bidEURStr.toFloat();
      Serial.print("Euro: ");
      Serial.print("Є ");
      Serial.println(euro, 2);

      // Bitcoin:
      String bidBTCStr = myObject["BTCBRL"]["bid"];
      float bitcoin = bidBTCStr.toFloat();
      Serial.print("Bitcoin: ");
      Serial.print("₿ ");
      Serial.println(bitcoin, 2);
